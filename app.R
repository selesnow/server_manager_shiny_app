...
server <- function(input, output, session) {
  ...
  observeEvent(logged_in(), {
    if (logged_in()) {
      ...
      # Добавляем ассистенту инструментарий для чтения листинга скриптов запускаемых планировщиком Windows
      dev_chat$register_tool(tool(
        get_task_script,
        "Получить листинг скрипта запускаемого определённой задачей из планировщика заданий Windows по названию задачи ",
        task_name = type_string(
          "Название задачи из планировщика заданий Windows по которой надо получить лог выполнения (Rout файл) запускаемого скрипта"
        )
      ))
      
      # Добавляем инструмент для того что бы ассистент мог получить информацию о любой задаче из планировщика Windows
      dev_chat$register_tool(tool(
        get_task_info,
        "Получить информацию об определённой задачей из планировщика заданий Windows по её названию.",
        task_name = type_string(
          "Название задачи из планировщика заданий Windows по которой надо получить лог выполнения (Rout файл) запускаемого скрипта"
        )
      ))
      
      # Добавляем ассистенту возможность запрашивать список упавших задач с краткой причиной
      dev_chat$register_tool(tool(
        get_failed_tasks,
        "Получить информацию о задачах из планировщика Windows, работа которых при прошлом запуске завершилась ошибкой. Функция вернёт название задач и код ошибки из планировщика."
      ))
      
      # Добавляет ассистенту возможность запросить список задач из планировщика заданий Windows
      dev_chat$register_tool(tool(
        get_tasks_csv,
        "Получить CSV таблицу с данными по задачам из панировщика заданий Windows. С их описанием и всеми параметрами. Эту функцию удобно использовать для поиска нужных задач по запросу пользователя."
      ))
      
      # Добавляем ассистенту возможность запускать задачи на сервере
      dev_chat$register_tool(tool(
        run_server_task,
        "Запуск задачи в планировщике заданий Windows на сервере. Важно при передаче аргумента task_name не добавлять в назании задачь лищние слешы.",
        task_name = type_string(
      ...
      # Анализ спринта аналитиков
      dev_chat$register_tool(tool(
        get_sprint_planfix_tasks,
        "Функция которая позволяет запросить задачи из Планфик (не из планировщика заданий Windows!) команды аналитиков, для анализа их спринта и успешности выполнения ими задач",
        year = type_string("Год в формате двухзначного числа (format(Sys.Date(), '%y')) если 2025 год то 25, если 2024 то 24, и так далее"),
        month = type_string("Месяц в формате двухзначного числа (format(Sys.Date(), '%m')) если январь то 01, если май то 05, и так далее"),
      ))
      
      # Формирование отчёта по юнит экономике
      dev_chat$register_tool(tool(
        unit_economics_analytics,
        "Функция для получения данных для расчёта дохода по Юнит Экономике. Каждая строка это отдельная задача, по которой мы заработали деньги (fact_cost). И на которую потратили определённое время (fact_hours).",
