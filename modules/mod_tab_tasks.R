...
chat <- ellmer::chat_gemini(
  system_prompt = str_glue(
    ...
    'В этом чате я тебе так же загружу README к моим внутренним пакетам, ',
    'используй их для ответы на вопросы если в логах будешь видеть функции из этих пакетов.'
  ), 
  echo = 'none'
)

# Загружаем в чат базовую документацию к внутренним пакетам
pac_rm <- dir('ai_docs')

for (rm in pac_rm) {
  
  pack_name <- str_replace(string = rm, '(.*)_README.md', '\\1')
  readme <- readLines(here::here('ai_docs', 'rpup_README.md'), warn = FALSE, encoding = "UTF-8")
  
  out <- chat$chat(
    glue::glue(
      'Ниже базовая документация к пакету {pack_name}, ',
      'ознакомься и используй её для аналища логов, когда в логаз встречаются функции из этого пакета:',
      '### {pack_name} readme',
      '{readme}'
    ), echo = F
  )
