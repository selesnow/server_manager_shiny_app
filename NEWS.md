...
### Added
* С помощью доступов из приложения можно скрывать кнопки и вкладки AI анализа, т.е. "Анализ Rout" и "Объясни код".
* На вкладку **Статистика** добавлены график и таблица, отображающие количетсво и процент активных задач в планировшике заданий Windows по году их запуска.
* На вкладку **Статистика** добавлены график и таблица, отображающие количество задач запускаемых по часам в сутках, для анализа нагрузки на сервер.
* В таблицу задач на вкладке **Задачи** добавлено поле `is_quite`, в котором помечаны отключенные от мониторинга задачи.
* На вкладке **Задачи** в области информации о задаче, и в диалоговом окне появляющемся по двойному клику по таблице задач добавлена информация о времени следующего запуска.

### Changed
* Из общей таблицы задач на вкладке **Задачи** убраны столбцы связанные с триггерами, таким образом из таблицы задач быи убраны все дублирования задач, а информация о триггерах хранится и подтягивается из отдельной таблицы.
* На вкладке **Задачи** информация о времени прошлого и следующего запука, а так же время создания процесса на вкладке **Службы** и **Процессы** теперь приводятся по Киевскому часовому поясу.
