...
# 2026-09-10 | GEMINI CLI (Alsey)
* Оптимизирован и доработан интерфейс AI чата на вкладке «AI Ассистент»:
  * Полностью восстановлен вертикальный, бесшовный макет на базе `bslib::page_fluid()`, а описание возможностей возвращено наверх страницы и оформлено в виде аккуратного блока `div.stats-description` без карточных оберток (в точности как на ветке `master`).
  * Чат-ассистент `chat_ui()` извлечен из любых `.card` и `.card-body` контейнеров. Из его CSS-стилей удалены все рамки, скругления углов, тени и фоны карточки. Чат теперь бесшовно сливается с фоном приложения (`--shiny-chat-bg: var(--bg-app)`), выглядя нативно и эстетично.
  * Высота чат-панели привязана к высоте экрана (`height: calc(100vh - 280px) !important`, `min-height: 520px !important`). Благодаря этому сообщения прокручиваются внутри самого чата, а **окно ввода запроса всегда зафиксировано в видимой нижней части экрана** и больше не уходит вниз.
  * Физическая кнопка **«Очистить чат»** извлечена из тесного встроенного тулбара чат-инпута и перемещена в просторный, аккуратно центрированный блок `div` прямо под чат-панелью.
  * Полностью решена проблема пустого списка слэш-команд при вводе `/` в окне запроса:
    * Сигнатуры обработчиков (`/failed`, `/help`, `/new`) переведены на функции без аргументов (`handler = function()`), что позволило автодополнению на клиенте мгновенно распознавать и запускать команды без ожидания дополнительного ввода.
    * Реализован фоновый самовосстанавливающийся таймер `reactiveTimer(4000)`, который каждые 4 секунды принудительно синхронизирует список слэш-команд с фронтендом, полностью побеждая гонку условий при медленной или динамической загрузке вкладок в браузере.
  * Исправлен баг в обработчике команды **`/failed`**: проверка упавших задач теперь надежно и безопасно сравнивает строковые типы данных через `identical()` (устранена ошибка `missing value where TRUE/FALSE needed`), выводя готовый лог ошибок или красивое праздничное сообщение напрямую в чат.
  * Версия cache-busting для всех CSS-файлов в `app.R` повышена до `?v=9`, принудительно обновляя кэш стилей у всех пользователей.
* Произведён переход на новую версию пакета `shinychat` 0.5.0 с встроенной историей нескольких диалогов:
  * Внедрён новый API `chat_ui()` и `chat_server()` взамен устаревших `chat_mod_ui()` и `chat_mod_server()`.
  * Интегрировано постоянное дисковое хранилище нескольких диалогов (Conversation History) с разделением контекста по авторизованным пользователям (`scope = usr_login`) и автоматическим сохранением в директории `chathistory/` (добавленной в `.gitignore`) через `FileConversationStore`.
  * Реализована нативная поддержка вложений (изображений, PDF-документов и файлов логов `.Rout`) напрямую в интерфейсе чата.
  * Интегрировано автоматическое сворачивание блока цепочки рассуждений (Reasoning/Thinking) модели `gemini-3.8-flash` над основным ответом.
  * Удалена старая физическая кнопка «Сбросить чат» в пользу встроенного в интерфейс `shinychat 0.5.0` механизма ведения и переключения чатов.
  * Реализован набор удобных горячих Slash-команд (`/`):
    * `/failed` — для автоматического поиска упавших задач в планировщике Windows прямо во время диалога.
    * `/help` — для вывода интерактивной справки по возможностям наших внутренних R-пакетов (`rpup`, `pfworker`, `n1`, `segments`, `serviceaccounts`).
    * `/new` — для ручного переключения на новую чистую вкладку беседы.
* Исправлена проблема с фиксированной высотой окна ввода промпта AI-ассистента: полностью переработана архитектура селекторов полей ввода в `www/css/main-styles.css`. Вместо глобального класса `.form-control` внедрены строгие точечные селекторы `textarea.form-control:not(shiny-chat-container textarea):not(shiny-chat-input textarea)`, которые гарантируют абсолютную изоляцию текстовых полей чата от наложения глобальных и мобильных CSS-свойств высоты и паддингов. Это вернуло логике `shinychat` на JS полную свобову в динамическом управлении высотой поля ввода.
* Для преодоления жесткого кэширования файлов стилей в браузерах обновлены версии cache-busting для всех CSS-файлов с `?v=3` на `?v=5` в главном файле `app.R`. Это гарантирует мгновенную загрузку обновленных стилей у всех пользователей.
* Добавлена безопасная стилизация цвета фона, текста и границ для полей ввода чата, позволяющая им идеально соответствовать теме Slate & Indigo (в темном и светлом режимах), не мешая при этом встроенным JavaScript-расчетам высоты.
* Обновлён пакет `ellmer` до версии `0.5.0` (с обновлением `renv.lock`) для исправления ошибки `HTTP 400 Bad Request` (пропущенный параметр `thought_signature`) при вызове инструментов ассистентом на базе модели `gemini-3.8-flash`.

## v1.37.3 - 2026-09-09

### Added
* Обновление на владке **Службы**:
  * Реализована подробная карточка службы (Popup Modal) на вкладке "Службы", открывающаяся при двойном клике на любую строку таблицы служб.
  * Внутри карточки выводится полная системная информация: название, отображаемое имя, статус (со стильным цветным бейджем), краткое описание, PID процесса, время старта и командные параметры запуска службы.
  * Внедрены удобные кнопки управления службой ("Запустить", "Остановить", "Перезапустить") непосредственно в футере карточки (доступно ролям с соответствующими привилегиями).
  * Все выполненные через карточку операции над службами исправно фиксируются в системном логе действий.

### Changed
* Обновление AI асситента:
  * Обновлена информация в системном промпте AI ассистента, обновление коснулись новых функций пакета `rpup` и доработки пакета `n1`.
  * Обновлена модель которую использует AI ассистент, теперь он работает на базе `gemini-3.8-flash`.

## v1.37.2 - 2026-09-08

### Added
* Реализована интерактивная карточка пользователя (Popup), которая открывается по двойному клику на строку в таблице пользователей на вкладке "Доступы".
* Внутри карточки выводится подробная информация: логин, текущая роль, дата регистрации (извлекается из логов добавления пользователей) и время последней активности (вычисляется по логам сессий и действий).
* Реализован встроенный блок полного управления пользователем прямо внутри модального окна: изменение роли, мгновенный сброс пароля с подтверждением и удаление аккаунта.
* Внедрена защита от случайного или небезопасного удаления системных аккаунтов (аккаунт root и собственный текущий сеанс администратора защищены от удаления).
* Все выполненные через карточку действия регистрируются в системном логе действий.

## v1.37.1 - 2026-08-31

### Fixed
* Исправлен конфликт стилей элементов `selectInput` (библиотека `selectize.js`), из-за которого вводимый текст поиска был белым на белом фоне. Поля выбора теперь полностью интегрированы с динамической темой Slate & Indigo в тёмном и светлом режимах.

## v1.37.0 - 2026-08-31

### Added
* Реализован современный, стильный и адаптивный дизайн в стиле SaaS Premium (Slate & Indigo) для всего Shiny-приложения на ветке `new_design`.
* Реализована полная мобильная адаптация (Mobile Responsiveness) интерфейса без изменения R-кода: добавлены медиа-запросы в CSS-стили для комфортной работы со смартфонов.
* Добавлена кастомная стилизация экранов загрузки пакета `waiter` (оверлеи `waiter_show()`): внедрен современный размытый фон (glassmorphism backdrop-filter blur), поддерживающий как светлую, так и темную тему, а спиннер SpinKit теперь анимируется с использованием основного цвета темы.
* Добавлена стандартная иконка профиля пользователя (через `icon("user")`) рядом с именем пользователя в шапке внутри панели `text-primary`.
* Добавлено управление доступом к кнопке просмотра кода «Код» и вкладке скриптов на панели логов (обновление от 2026-08-11).

### Changed
* Полностью обновлен файл стилей `www/css/main-styles.css`: добавлены Google Fonts ('Plus Jakarta Sans'), улучшена палитра цветов для темной и светлой тем, скруглены карточки (12px), добавлены плавные hover-эффекты и тени.
* Оптимизированы навигационные вкладки на мобильных устройствах: теперь они выстраиваются в горизонтальную свайп-ленту (scroll-swipe) с плавным скроллом, предотвращая переносы строк.
* Улучшена адаптивность шапки (вертикальный стек с адаптивной сеткой кнопок), таблиц Datatables, инпутов календаря и пузырей чата AI-Ассистента на мобильных экранах.
* Скорректировано выравнивание кнопок управления в шапке: кнопки "Обновить данные", "Светлая тема" и "Выйти" теперь корректно прижаты к левой стороне под именем пользователя, сохраняя компактную структуру.
* Обновлена стилизация таблиц DT (Datatables): добавлена кастомная пагинация, округленный поиск и интерактивная подсветка строк при наведении.
* Переработан интерфейс чата AI-Ассистента: сообщения пользователя и ассистента теперь представлены в виде стильных bubble-сообщений, а таблицы в чате получили контрастное оформление.
* Переписан файл `www/css/header-styles.css`: панель заголовка стилизована под элегантную навигационную панель, а имя пользователя оформлено в виде индиго-бейджа.
* Модернизирован календарь в `www/css/calendar.css` с полной поддержкой динамической смены темной и светлой тем.
* Улучшен интерфейс авторизации на экране входа (`modules/mod_auth.R`) с использованием Glassmorphism эффекта и радиального фона.
* Внесены изменения в `app.R`: шрифты и CSS-ресурсы с параметрами cache-busting (?v=3) подключены глобально на уровне статического UI, убран дублирующий код загрузки стилей из renderUI.

## v1.36.3 - 2026-08-11
- Добавлено управление доступом к кнопке просмотра кода «Код» и вкладке скриптов на панели логов.

## v1.36.2 - 2026-05-22

### Fixed
* Исправлена критическая ошибка "object datatables_html not found", возникавшая при логине под ролью viewer. Ошибка была вызвана несоответствием сигнатуры функции `forget_task_na` при регистрации инструментов AI-ассистента.

## v1.36.1 - 2026-04-29

### Changed
* Приложение адаптировано на обновлённую версию R 4.6.0 и обновление всех пакетов, критические изменения коснулись пакета `purrr`.

## v1.36.0 - 2026-02-12

### Added
* В приложении добавлен авто логин, т.е. теперь прилодение после успешной авторизации сохраняет в браузере авторацию на 30 дней.

### Changed
* Обновлена логика зависимости фильтров на вкладке **Задачи**: реализована полная взаимозависимость фильтров. Доступные опции теперь пересчитываются «на лету» без сброса текущего выбора пользователя, что упрощает поиск задач по нескольким критериям. 

### Fix
* Исправил ошибку, изза которой при сбросе чата на вкладке с ассистентом сама история чата визуально не очищалась.
...
### Changed
* На вкладке **Задачи** внедрена касскадная зависимость фильтров.
* Обновлены пакеты:
  * `shinychat` 0.2.0.9000 -> 0.3.0.9000
  * `ellmer` 0.3.2 -> 0.4.0

### Added
* В системный промпт добавлено описание новых функций из пакета `pfworker`.

## v1.35.1 - 2025-11-19

### Fix

* Восстановлена работа кнопки "Выгрузить в Google Sheets" на вкладке **Задачи**.

## v1.35.0 - 2025-11-19

### Added

* В проект внедрён `renv` для изоляции среды приложения и удобного управления зависимостями.

## v1.34.1 - 2025-10-16

### Fixed

* В приветственное сообщение AI-ассистента возвращено **обращение по логину пользователя**.

## v1.34.0 - 2025-10-15

### Added

* Реализовано **полное логирование диалогов** с AI-ассистентом.
* Добавлено **логирование поиска по файлам**.
* Добавлено **логирование выполнения команд**
* Вкладка **Логи** теперь разделена на четыре под-вкладки:

  * **Основные** — прежний функционал вкладки.
  * **AI-чат** — просмотр логов всех диалогов с ассистентом.
  * **Поиск по файлам** — просмотр логов результатов поиска.
  * **CMD** - просмотр логов выполнения команд на вкладке CMD.

## v1.33.0 - 2025-10-01

### Added
* Вкладка *Логи* существенно доработана:  
  * появилась возможность группировать данные по дням, неделям и месяцам;  
  * добавлен график количества уникальных пользователей по дням;  
  * добавлен график активности пользователей по дням недели и времени суток;  
  * добавлена сводная таблица со статистикой по периодам.  
* AI ассистенту добавлен инструмент для отключения задач от мониторинга
* На вкладке **Доступы** добавлен отдельный пункт *Управление мониторингом задач* для разрешения отдельным ролям управления мониторингом задач.

### Changed
* Интерфейс AI-ассистента переведён на `shinychat` версии `0.2.0.9000`:  
  * теперь видно, какой инструмент с какими аргументами вызывает модель;  
  * при сбросе чата удаляется вся история диалога.  
* Оптимизирован поиск AI-ассистентом логов и скриптов, запускаемых задачами.  
* Улучшено описание функций пакета `rpup` в системном промпте AI-ассистента.  
* На вкладке *CMD* расширена строка ввода команд и увеличена кнопка «Выполнить».  

## v1.32.6 - 2025-09-24

### Added
* Добавлены настройки доступа, позволяющие скрывать кнопки и вкладки AI-анализа, такие как **Анализ Rout** и **Объясни код**.
* На вкладку **Статистика** добавлены график и таблица, показывающие количество и процент активных задач в планировщике Windows по годам их запуска.
* На вкладку **Статистика** добавлены график и таблица, отображающие распределение запусков задач по часам суток для анализа нагрузки на сервер.
* В таблицу задач на вкладке **Задачи** добавлено поле `is_quite`, отмечающее задачи, исключённые из мониторинга.
* На вкладке **Задачи** в области информации о задаче, а также в диалоговом окне (вызываемом двойным кликом по задаче) теперь отображается время следующего запуска.

### Changed
* Из общей таблицы задач на вкладке **Задачи** убраны столбцы, связанные с триггерами. Это устранило дублирование: теперь информация о триггерах хранится и подтягивается из отдельной таблицы.
* На вкладке **Задачи** (время последнего и следующего запуска), а также во вкладках **Службы** и **Процессы** (время создания процесса) все даты и время приводятся к Киевскому часовому поясу.

## v1.32.5 - 2025-09-21

### Added
На вкладку **Задачи** в блок с информацией о задаче, и popup добавлена информация о триггерах задачи.
