...
* Для преодоления жесткого кэширования файлов стилей в браузерах обновлены версии cache-busting для всех CSS-файлов с `?v=3` на `?v=5` (и до `?v=9` в ходе финальной оптимизации) в главном файле `app.R`. Это гарантирует мгновенную загрузку обновленных стилей у всех пользователей.
...
### Fixed
* Исправлена проблема с переполнением и выходом длинных названий задач за пределы интерфейса: поле выбора `selected_task` теперь имеет адаптивную ширину `100%` и плавно скрывает длинные пути с помощью троеточия (ellipsis), а Popup-модальное окно автоматически переносит заголовок и название задачи благодаря флекс-контейнерам и свойству `word-break: break-all`.
